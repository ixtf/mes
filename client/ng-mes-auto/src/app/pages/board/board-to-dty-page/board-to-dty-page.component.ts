...
import {BoardToDtyPageState, GroupByBatchGradeItem, InitAction, SilkCarRuntimeReportItem, UpdateSilkCarRuntimeEvent} from './board-to-dty-page.state';

declare const EventBus: any;

@Component({
  templateUrl: './board-to-dty-page.component.html',
  styleUrls: ['./board-to-dty-page.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BoardToDtyPageComponent implements OnInit, OnDestroy {
  @Select(BoardToDtyPageState.timeOutSilkCarRuntimes)
  readonly timeOutItems$: Observable<SilkCarRuntimeReportItem[]>;
  @Select(BoardToDtyPageState.groupByBatchGradeItems)
  ...
  constructor(private store: Store,
    ...
    this.eb.onopen = () => {
      this.eb.registerHandler('mes-auto://websocket/boards/toDyt/refresh', this.refresh);
      this.eb.registerHandler('mes-auto://websocket/boards/toDyt/events', this.updateSilkCarRuntimeEvent);
    };
  }
...
@NgModule({
  declarations: [
    BoardToDtyPageComponent,
  ],
  imports: [
    NgxsModule.forFeature([BoardToDtyPageState]),
    SharedModule,
    RouterModule.forChild([
      {path: '', component: BoardToDtyPageComponent},
    ]),
  ],
})
export class Module {
