...
const PAGE_NAME = 'BoardToDtyPage';

export class InitAction {
  static readonly type = `[${PAGE_NAME}] InitAction`;

  constructor(public payload: { destinationId: string; }) {
  }
}
...
export class BoardToDtyPageState {
  ...
  static timeOutSilkCarRuntimes(state: StateModel): SilkCarRuntimeReportItem[] {
    return BoardToDtyPageState.allItems(state).filter(it => {
  ...
  static groupByBatchGradeItems(state: StateModel): GroupByBatchGradeItem[] {
    ...
    BoardToDtyPageState.allItems(state).forEach(item => {
  ...
  @Action(InitAction)
  @ImmutableContext()
  InitAction({getState, setState, dispatch}: StateContext<StateModel>, {payload: {destinationId}}: InitAction) {
  ...
  RefreshAction(ctx: StateContext<StateModel>) {
    const {destinationId, silkCarRuntimeFilter} = ctx.getState();
