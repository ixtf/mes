...
const PAGE_NAME = 'ToDtyConfirmReportPage';
...
interface StateModel {
  ...
  itemEntities: { [operatorId: string]: ToDtyConfirmReportItem };
}
...
@Injectable()
export class ToDtyConfirmReportPageState {
  ...
  @Selector()
  @ImmutableSelector()
  static items(state: StateModel): ToDtyConfirmReportItem[] {
    return ToDtyConfirmReportPageState.operators(state).map(it => state.itemEntities[it.id]);
  }

  @Selector()
  @ImmutableSelector()
  static totalItemMap(state: StateModel): ToDtyConfirmReportItem {
    const ret = {silkCount: 0, silkCarRecordCount: 0};
    ToDtyConfirmReportPageState.items(state).forEach(item => {
      ret.silkCarRecordCount += item.silkCarRecordCount;
      ret.silkCount += item.silkCount;
    });
    return ret as ToDtyConfirmReportItem;
  }
  ...
  QueryAction({setState}: StateContext<StateModel>, {payload: {workshopId, startDateTime, endDateTime}}: QueryAction) {
    ...
    return this.api.toDtyConfirmReport({workshopId, startDateTime: `${moment(startDateTime).valueOf()}`, endDateTime: `${moment(endDateTime).valueOf()}`}).pipe(
