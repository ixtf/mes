...
export class DyeingResultsTimelinePageState {
  ...
  @Action(InitAction)
  @ImmutableContext()
  InitAction({dispatch}: StateContext<StateModel>, {payload: {lineMachineId, spindle}}: InitAction) {
    if (lineMachineId && spindle) {
      return dispatch(new QueryAction({lineMachineId, spindle}));
    }
  }

  @Action(QueryAction)
  @ImmutableContext()
  QueryAction({setState}: StateContext<StateModel>, {payload: {lineMachineId, spindle}}: QueryAction) {
    const httpParams = HTTP_PARAMS.set('lineMachineId', lineMachineId).set('spindle', `${spindle}`);
    ...
    return forkJoin([firsts$, crosses$]).pipe(
      tap(([firsts, crosses]) => setState((state: StateModel) => {
        state.lineMachineId = lineMachineId;
        state.spindle = spindle;
  ...
  MoreAction({setState, getState}: StateContext<StateModel>, {payload: {type}}: MoreAction) {
    const {lineMachineId, spindle} = getState();
    const httpParams = HTTP_PARAMS
      .set('type', type)
      .set('lineMachineId', lineMachineId)
      .set('spindle', `${spindle}`)
