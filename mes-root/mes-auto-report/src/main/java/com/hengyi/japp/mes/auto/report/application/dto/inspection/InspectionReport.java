package com.hengyi.japp.mes.auto.report.application.dto.inspection;
...
public class InspectionReport {
    private static final Collection<String> PRODUCT_PROCESS_IDS = Set.of("5c0084c751e9c40001573af7", "5c0084d951e9c40001573d43");
    ...
    public InspectionReport(String workshopId, long startDateTime, long endDateTime, Collection<String> silkCarRecordIds) {
    ...
    public static InspectionReport create(String workshopId, long startDateTime, long endDateTime) {
        ...
        return new InspectionReport(workshopId, startDateTime, endDateTime, ids);
    }
        ...
        public JsonNode toJsonNode() {
            final Collection<GroupBy_Product> products = Flux.fromIterable(batchMap.values())
                    .reduce(Maps.<Product, GroupBy_Product>newConcurrentMap(), (acc, cur) -> {
                        acc.compute(cur.getBatch().getProduct(), (k, v) -> Optional.ofNullable(v).orElse(new GroupBy_Product(k)).collect(cur));
                        return acc;
